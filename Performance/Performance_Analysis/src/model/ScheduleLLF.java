...
public class ScheduleLLF {
	...
	public ScheduleLLF(ArrayList<Task> tasks) {
	...
	private int chooseInstance(int currentTime, ArrayList<Integer> index) {
		int target = -1, min = Integer.MAX_VALUE;
		...
		if (currentInstance == -1) {
			...
			return target;
		} else if ((tListProcessed.get(currentInstance).getP()
				- tListProcessed.get(currentInstance).getC()) == (tListProcessed.get(target).getP()
						- tListProcessed.get(target).getC())
	...
	private int simTime(ArrayList<Task> tasks, int start, int end) {
		if ((end - start) == 1)
			return lcm(tasks.get(start).getP(), tasks.get(end - 1).getP());
		else
			return (lcm(tasks.get(start).getP(), simTime(tasks, start + 1, end)));
	}
	...
	////////////////////////////////////////////////////////////////////////////////////////////////
	// Calculate number of pre-emptions:
	////////////////////////////////////////////////////////////////////////////////////////////////
	public int calNumPreEmption() {
		int numPreEmption = 0;
		for (int i = 1; i < idArr.length; i++) {

			// Previous task id is not current task id
			if (idArr[i] != idArr[i - 1]) {
				// either previous or current task is an idle task
				if (idArr[i] != -1 || idArr[i - 1] != -1) {
					// previous task is not done
					if (!isTaskDone(idArr[i - 1], instanceArr[i - 1], (i - 1))) {
						numPreEmption++;
					}
				}
			}
	...
	private boolean isTaskDone(int taskId, int taskInstanceId, int taskIndex) {
		for (int i = taskIndex + 1; i < idArr.length; i++) {
			if (taskId == idArr[i] && taskInstanceId == instanceArr[i]) {
				return false;
			}
		}
		return true;
